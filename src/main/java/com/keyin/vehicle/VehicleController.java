...
//    }
    ...
    public List<Vehicle> searchVehicles(
            ...
            @RequestParam Long agency_pk,
            @RequestParam(required = false) String manufacturer,
            @RequestParam(required = false) String model) {
        VehicleSearchCriteria criteria = new VehicleSearchCriteria();
        criteria.setCategory(category);
        criteria.setAgencyPk(agency_pk);
        criteria.setManufacturer(manufacturer);
        criteria.setModel(model);
        return service.searchVehicles(criteria);
    }

    @GetMapping("/vehicles/search")
    public List<Vehicle> searchVehicle(@RequestParam(value = "agency", required = false) long pk, @RequestParam(value = "category", required = false) String category) {
        return service.findVehicleByAgencyAndCategory(pk, category);
    }
}
